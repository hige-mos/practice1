...
window.onload = function(){
	...
	fardistance = {'ss4': 12,'ss5': 5,'ss6': 8}//edit
	dislabelset = ['much close', 'close', 'a little close', 'middle', 'a little far', 'far', 'much far']
...
function construct(){
	...
	for(var i = 0 ; i < ElementsCount ; i++ ) {
		if (input.role[i].checked ==true){
			showrolelist.push(input.role[i].value);
		}
	}
...
function Scenario5(){
	...
	for(control of controllist){
		...
		for(var node in NODE){
			...
			}else if(NODE[node]['act'] == 'STA' && showrolelist.indexOf(NODE[node]['role']) != -1){	// Active STA
				chartx.selectAll('#'+ ss+"_"+control+"_"+drop+"_"+node+ "_Plot")
					.data([{x: x(NODE[node].x), y: y(NODE[node].y)+z(NODE[node].z)}])
					.enter()
					.append('circle')
...
function ToolTipInfo(str_id) {
	var id = str_id.split("_")
	var nodeset = dataset[id[1]][id[0]][id[2]][Number(id[3])]
	var str = ''
	table = []
	table.unshift('<table border = "1"><tbody>')
	table.push('</tbody></table>')
	table.splice(table.length -1 ,0 ,'<th colspan = "2"><h5>Detailed Info.</h5></th>')
	table.splice(table.length -1 ,0 ,'<td>Node No.</td><td>' + nodeset['node']  + '</td>')
	table.splice(table.length -1 ,0 ,'<td>Association</td><td>' + nodeset['assoc']  + '</td>')
	table.splice(table.length -1 ,0 ,'<td>Apprecation</td><td>' + nodeset['role']  + '</td>')
	table.splice(table.length -1 ,0 ,'<td>Position</td><td>[' + nodeset['x']  + ',' + nodeset['y'] + ',' + nodeset['z']  + ']</td>')
	table.splice(table.length -1 ,0 ,'<th colspan = "2">Downlink</th>')
	table.splice(table.length -1 ,0 ,'<td>Throughput</td><td>' + nodeset['Dthr'] + ' Mbps </td>')
	table.splice(table.length -1 ,0 ,'<td>MCS</td><td>' + nodeset['Dmcs'] + '</td>')
	table.splice(table.length -1 ,0 ,'<td>SINR</td><td>' + nodeset['Dsinr'] + '</td>')
	table.splice(table.length -1 ,0 ,'<th colspan = "2">Uplink</th>')
	table.splice(table.length -1 ,0 ,'<td>Throughput</td><td>' + nodeset['Uthr'] + ' Mbps </td>')
	table.splice(table.length -1 ,0 ,'<td>MCS</td><td>' + nodeset['Umcs'] + '</td>')
	table.splice(table.length -1 ,0 ,'<td>SINR</td><td>' + nodeset['Usinr'] + '</td>')
	table.splice(table.length -1 ,0 ,'<th colspan = "2">Time</th>')
...
function ShowNodeInfo(id) {
	for(var control of controllist){
		var node = Number(id[3]),PLOT = NODE[node],sbss = 0, aps = 0, hid = 0;
		for(var detect of PLOT.inter){
			if(showrolelist.indexOf(NODE[detect].role) != -1){
				...
				if(NODE[detect].act == 'AP'){
					...
					p.style("stroke", "black")
					sbss++;
				}else if(NODE[PLOT.bss].inter.indexOf(detect) == -1){	// hidden terminal for UL
					p.style("stroke", "blue");
					hid++;
...
function Evalplot(ss){
	...
	evalchart = svg.append("g")
		.attr("id", "evalchart")
		.attr("transform", translate(cwidth + margin.left , height + margin.space * 6));
...
function STAplot(ss, drop){
	...
	STAchart = svg.append("g")
		.attr("id", "STAchart")
		.attr("transform", translate((width + margin.space) * 2,margin.space));
	STACbarchart = STAchart.append("g")
		.attr("id", "STACbarchart")
		.attr("transform", translate(margin.space,0));
	STALeftchart = STAchart.append("g")
		.attr("id", "STALeftchart")
		.attr("transform", translate(cwidth + margin.left + margin.space,margin.top));
	STARightchart = STAchart.append("g")
		.attr("id", "STARightchart")
		.attr("transform", translate(cwidth + margin.left + Pwidth + margin.space * 2,margin.top));
	STAchartlist = [STALeftchart, STARightchart]
	//Title
	STAchart.append("text")
		.attr("transform", translate(cwidth + margin.left + Pwidth + margin.space,-margin.space/2))
		.attr("text-anchor", "middle")
		.style("font-size", "50px")
		.text("STA Plot");
	// axis scale
	...
	for(var control of controllist){
		...
		for(var node in STAdset[control]){
			if (showrolelist.indexOf(NODE[node]['role']) != -1){
...
function APplot(ss, drop){
	...
	APchart = svg.append("g")
		.attr("id", "APchart")
		.attr("transform", translate((width + margin.space) * 2, margin.space * 3 + Pwidth));
